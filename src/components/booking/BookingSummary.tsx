import { CheckCircle, Calendar, Clock, User, Briefcase, CalendarPlus } from "lucide-react";
...
// Función helper para crear el link dinámico de Google Calendar
const generateGoogleCalendarLink = (
  ...
  // Formato UTC que requiere Google (YYYYMMDDTHHmmssZ)
  const formatDateForGoogle = (d: Date) => d.toISOString().replace(/-|:|\.\d\d\d/g, "");
    ...
    <Card className="border-border shadow-sm">
      <CardContent className="p-6 space-y-5">
        ...
        <div className="space-y-3 text-sm bg-muted/30 p-4 rounded-xl border border-border/50">
          <h4 className="font-semibold text-foreground text-base mb-4">{businessName}</h4>
        ...
        {confirmed && (
          <div className="space-y-3 pt-2">
            ...
            >
              <a 
                href={generateGoogleCalendarLink(service.nombre_servicio, businessName, date, time, service.duracion_min)} 
                target="_blank" 
